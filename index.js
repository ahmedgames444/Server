...
client.on('ready', async () => {
    ...
    client.user.setActivity(`Ahmed`, { type: "PLAYING" })
    ...
    for (const file of commandFiles) {
        ...
        client.api.applications(client.user.id).guilds('782636340096401418').commands.post({ data: {
            name: command.name,
            description: command.description,
            options: command.commandOptions
        }}) //لا تنسا تحط ايدي سيرفر
    ...
    let cmdArrGuild = await client.api.applications(client.user.id).guilds('699704537287753789') .commands.get() // لا تنسا تحط ايدي سيرفر
